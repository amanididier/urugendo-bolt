...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    fetchTrips(search.from, search.to, search.date)
      .then((dbTrips) => {
        if (cancelled) return;
        if (dbTrips.length > 0) {
          setTrips(dbTrips);
        } else {
          setTrips(getTripsForRoute(search.from, search.to, search.date));
        }
      })
    ...
    <div className="bg-surface-secondary pb-[100px] min-h-full">
      <div className="bg-primary pt-[56px] px-5 pb-5 rounded-b-[28px] relative overflow-hidden">
        ...
        <div className="relative flex items-center gap-3">
          <button onClick={() => router.push('/home')} className="p-1 -ml-1 active:scale-90 transition-transform">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center px-6 py-16">
          <div className="text-4xl mb-4">🚏</div>
          <p className="text-[15px] text-text-muted text-center">No trips found for this route on this date.</p>
        </div>
      ) : (
        <div className="px-5 space-y-3 mt-2">
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-3">
                  <div className="flex-1 min-w-0">
                    <div className="text-[28px] font-black text-text-primary leading-none">{trip.departureTime}</div>
                  ...
                  <div className="flex-1 min-w-0 text-right">
                    <div className="text-[28px] font-black text-text-primary leading-none">{trip.arrivalTime}</div>
